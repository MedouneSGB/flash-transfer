...
![Flash Transfer](https://img.shields.io/badge/version-1.6.2-FFD700?style=for-the-badge&logo=lightning&logoColor=black)
...
├── nsis/          → Flash Transfer_1.6.2_x64-setup.exe  (Windows)
├── msi/           → Flash Transfer_1.6.2_x64.msi        (Windows)
├── dmg/           → Flash Transfer_1.6.2_x64.dmg        (macOS Intel)
├── dmg/           → Flash Transfer_1.6.2_aarch64.dmg    (macOS Apple Silicon)
├── appimage/      → flash-transfer_1.6.2_amd64.AppImage (Linux)
└── deb/           → flash-transfer_1.6.2_amd64.deb      (Linux)
```
...
```bash
npm run version:patch   # 1.6.2 -> 1.5.1
npm run version:minor   # 1.6.2 -> 1.6.2
npm run version:major   # 1.6.2 -> 2.0.0
